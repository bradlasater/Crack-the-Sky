...
Lives in ``ingest.common`` rather than ``pricing`` so the ingest jobs can use
it without importing the pricing stack (which carries numpy/scipy). It is
re-exported from :mod:`pricing.rates` for the pricing API surface.
...
def load_curve(
    ...
    # Every partition must be scanned. `dt=` is the *ingestion run date*, not
    # the curve date: a resumed `--full` walk writes progressively older
    # history into progressively newer partitions, so the newest partition can
    # hold 1962 rows while an older one holds last week's curve. Stopping at
    # the first partition with any match would then return a decades-stale
    # curve for a current quote.
