...
- `ingest/common/cli.py` — **fixed**: reserved keys (`rows`, `bytes`, `job`,
  `duration_s`) are dropped from the `**extras` merge so a successful job
  cannot crash `job_end` and get reported as `job_error`.
